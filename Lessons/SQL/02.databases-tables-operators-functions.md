...
`EXISTS`|See below|TRUE if the subquery returns at least 1 record
...
Use **`EXISTS`** to check if there's any Singer with Sale > 2 000 000
...
The equivalent with **`ANY`**: 

```sql
SELECT * FROM Artists
WHERE 2300000 = ANY (SELECT Sale FROM Artists);	-- The whole expression turns TRUE and basically drops any restrictions
```
