...
A newer and better way to do pipes. It forwards errors and cleans up automatically. It's loaded from the `stream` module.
...
pipeline(process.stdin, process.stdout, (err) => console.error)
```

With **promisification** and **transforming stream**:
...
async function run() {
	try {
		await pipeline(
			...
			zlib.createGzip() /* transforming function */,
