...
2. Add the mehods:
   ...
   3. **`_construct (cb)`** - used to initialize resources before the stream is used
...
**None** of the mentioned above private function should be called directly - they will be called by the stream itself.

---

**Example**: create a Writable stream that adds `!!!` to the end of each chunk while appending them to the file.
...
To create more instances of our custom streams easily, we can create a class for it:
